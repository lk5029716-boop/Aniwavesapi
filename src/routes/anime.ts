...
// Temporary debug endpoint for DGHG testing
router.get("/debug/dghg-test", async (req, res): Promise<void> => {
  ...
  try {
    ...
    // Fetch the embed page and show what we get
    ...
    // Try extraction
    const stream = await extractDghg(embedUrl);
